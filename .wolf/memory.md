...
| 10:40 | Session end: 12 writes across 9 files (AttendanceServiceReconciliationTest.java, SessionProjector.java, SessionProjectorTest.java, V11__session_projection.sql, Attendance.java) | 1 reads | ~14751 tok |

## Session — 2026-09-09 — Server-side normalization (PR #1, #2)
Root cause of the recurring double check-ins (БИСЕР 08.09, ДАНИЕЛ 02.09): the TERMINAL decided check-in vs check-out from a local guess. Accept+flag (anomalies) only moved the work to the admin, so the user asked for it to be impossible instead.
Replaced with server-side normalization: terminal only reports "worker seen at time T"; the server derives `type` by projecting the whole day's ordered events. SessionProjector is a pure function (12 tests incl. order-independence over 25 shuffles + idempotence) — those two properties are what make offline sync correct. renormalizeDay() re-projects a (worker,site,day) after every sync batch, so a late offline batch heals the day. V11 adds client_type (raw reported direction) + ignored/ignored_reason; nothing is ever deleted. Three rules collapsed into ONE threshold: two scans <15 min apart = accidental re-scan (attendance.min-gap-minutes). Real check-out supersedes the scheduler auto-checkout; ADMIN_MANUAL rows are authoritative and never re-typed. Report/terminal/scheduler queries skip ignored rows. UI: anomaly badge+filter removed, single green/red button driven by the SERVER status refreshed on face detection, Details shows "terminal sent X -> stored Y". PR #2 removed the then-dead Аномалия column from all three Excel sheets + dead DTO/interface fields.
Verified end-to-end against a real stack: old-client payload with a WRONG direction was corrected server-side (backwards compatible — devices can update whenever they regain connectivity); late offline 08:00 event re-projected the already-stored 12:00 record from CHECK_IN to CHECK_OUT. Backend 18/18, frontend tsc clean. Historical rows intentionally untouched.
| 10:41 | Session end: 12 writes across 9 files (AttendanceServiceReconciliationTest.java, SessionProjector.java, SessionProjectorTest.java, V11__session_projection.sql, Attendance.java) | 1 reads | ~14751 tok |
| 12:29 | Session end: 12 writes across 9 files (AttendanceServiceReconciliationTest.java, SessionProjector.java, SessionProjectorTest.java, V11__session_projection.sql, Attendance.java) | 1 reads | ~14751 tok |
| 12:33 | Session end: 12 writes across 9 files (AttendanceServiceReconciliationTest.java, SessionProjector.java, SessionProjectorTest.java, V11__session_projection.sql, Attendance.java) | 1 reads | ~14751 tok |
| 12:37 | Session end: 12 writes across 9 files (AttendanceServiceReconciliationTest.java, SessionProjector.java, SessionProjectorTest.java, V11__session_projection.sql, Attendance.java) | 1 reads | ~14751 tok |
| 12:47 | Session end: 12 writes across 9 files (AttendanceServiceReconciliationTest.java, SessionProjector.java, SessionProjectorTest.java, V11__session_projection.sql, Attendance.java) | 1 reads | ~14751 tok |
| 12:50 | Session end: 12 writes across 9 files (AttendanceServiceReconciliationTest.java, SessionProjector.java, SessionProjectorTest.java, V11__session_projection.sql, Attendance.java) | 1 reads | ~14751 tok |
| 12:52 | Session end: 12 writes across 9 files (AttendanceServiceReconciliationTest.java, SessionProjector.java, SessionProjectorTest.java, V11__session_projection.sql, Attendance.java) | 1 reads | ~14751 tok |

## Session — 2026-09-09 (cont.) — Guards / 12-24h shifts (PR #3, merged as 531cca7)
Requirement: 4-5 guards work 12/24h shifts outside working hours and must be excluded from auto-checkout. User proposed a GUARD *role*; I found that would break things — FaceDescriptorService gates face registration on role == WORKER (guards could not check in at all) plus 3 more role == WORKER branches in UserService. Since guards have identical permissions, it is not a permissions concept → used a separate attribute ShiftType (DAY | SHIFT_24H). V12 defaults every existing user to DAY; the API treats null as "keep current" so an older client cannot reset a guard.
Excluding from auto-checkout alone was NOT enough: SessionProjector resets at midnight, so a 19:00→07:00 shift produced two check-ins and zero hours. Three coordinated changes: (1) scheduler query filters out SHIFT_24H; (2) SessionProjector.project() takes an initialExpected, and renormalizeDay seeds it from the last kept event before the day (48h lookback) so the first scan of a new day CLOSES the open shift — sync also re-projects the following day; (3) ReportService dates a shift worker's records by the session that opened them instead of the calendar day.
Self-audit before merge found 2 more real gaps: a single-day report showed NOTHING for a finished overnight guard shift (check-in before the window, check-out after the 06:00 boundary) → worked-hours window widened a day both sides + trimToRange; and a forgotten check-out could be closed by an unrelated scan 2 days later into a ~58h session → only continue a session younger than MAX_OPEN_SHIFT_HOURS (26h). Plus: memoised isShiftWorker per batch, ignored filter added to the remaining 6 dashboard queries, guard badge in the workers list.
Guard safety net = alert, not auto-close (user chose option B): GuardShiftMonitor runs daily 09:00 and notifies admins about shifts open >26h; the dashboard shows the same list live via openGuardShifts. Deliberately never closes — a shift may be 12h or 24h, and an invented end time looks like real payroll data.
Verified e2e on a real stack: single-day report → in=19:00 out=07:00 hours=12.0; auto-checkout returned {"created":0} for a guard; overnight scan stored as CHECK_OUT; dashboard returned the open-shift alert. 23/23 backend tests, tsc clean.
