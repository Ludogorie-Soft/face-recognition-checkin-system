 ...
 * inferredCheckOut is LEGACY and now always false. It used to mark a check-out guessed from the
 *   worker's next CHECK_IN at a different site — a workaround for pairing sessions per site. Since
 *   sessions are paired per worker, a scan at another site is a real CHECK_OUT and pairs normally.
 *   The field and its UI marker are kept only so older clients keep rendering.
